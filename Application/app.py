...
@app.route("/chinese/lat")
def chinese_lat():
    """Return a list of sample names."""

    # Use Pandas to perform the sql query
    stmt = db.session.query(Chinese_Table).statement
    df = pd.read_sql_query(stmt, db.session.bind)

    # Return a list of the column names (sample names)
    return jsonify(list(df["Place Latitude"]))

@app.route("/chinese/lon")
def chinese_lon():
    """Return a list of sample names."""
    ...
    # Return a list of the column names (sample names)
    return jsonify(list(df["Place Longitude"]))
